...
const EmailItem = ({ item }: { item: EmailData }) => {
  return (
    <Pressable
      ...
      <XView style={styles.emailContent}>
        <Avatar source={{ uri: item.avatarUrl }} fallbackInitials={item.name.slice(0, 2)} size="medium" />
        <YView style={styles.emailDetails}>
          <XView style={styles.emailHeader}>
          ...
          {item.tags.length > 0 && (
            <XView style={styles.tagsContainer}>
